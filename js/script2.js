...
const bloques = document.querySelectorAll('.bloque-area');
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const contenedor = document.querySelector('.contenedor-areas');

  if (!contenedor) return;

  const colores = [
    getComputedStyle(document.documentElement).getPropertyValue('--azul'),
    getComputedStyle(document.documentElement).getPropertyValue('--azul-claro'),
    getComputedStyle(document.documentElement).getPropertyValue('--coral'),
    getComputedStyle(document.documentElement).getPropertyValue('--coral-claro'),
    getComputedStyle(document.documentElement).getPropertyValue('--coral-oscuro')
  ];
  ...
  for (let i = 0; i < 40; i++) {
    ...
    bubble.style.background = `
      radial-gradient(circle, ${color}, rgba(255,255,255,0.02))
    `;
