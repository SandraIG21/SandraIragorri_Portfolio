...
window.addEventListener('scroll', () => requestAnimationFrame(updateCursorPosition), { passive: true });
window.addEventListener('wheel', () => requestAnimationFrame(updateCursorPosition), { passive: true });
...
//Fotografía Alisau
const contenedor = document.querySelector(".fotografia-carrusel .carrusel-contenedor");
const btnPrev = document.querySelector(".fotografia-carrusel .prev");
const btnNext = document.querySelector(".fotografia-carrusel .next");
const imgs = document.querySelectorAll(".fotografia-carrusel .carrusel-contenedor img");
...
function actualizarCarrusel() {
  const margen = 10; 
  const ancho = imgs[0].clientWidth + margen;
  contenedor.style.transform = `translateX(-${indiceInicio * ancho}px)`;

  // Controlar visibilidad de botones
  btnPrev.disabled = indiceInicio <= 0;
  btnNext.disabled = indiceInicio >= total - visibles;
  ...
  // Botón next
  if (indiceInicio >= total - visibles) {
